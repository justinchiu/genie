...
class CrnnLmEsqca(LvmA):
    def __init__(
        ...
        super(CrnnLmEsqca, self).__init__()
        ...
        if self.glove:
            ...
            if self.Vx.vectors is not None:
                # during test don't need to load in vectors
    ...
    def forward(
        ...
        v2dgx = (
            self.lutgx(
                ...
                .rename("x", "rnns") if self.glove else None
        )
        ...
        if not self.glove and self.bil:
            vc = self.Wvy(vc.rename("rnns", "ctxt")).rename("ctxt", "rnns")
        elif not self.glove and self.mlp:
            vc = self.Wvy1(self.Wvy0(vc.rename("rnns", "ctxt")).tanh()).tanh().rename("ctxt", "rnns")
        ...
        soft_past = (
            pc.get("copy", 0) * rnn_o + pc.get("copy", 1) * vc
        ).repeat("k", 1).rename("rnns", "ctxt")
        ...
        if self.noattnvalues:
            ...
            if self.tanh:
                past = past.tanh()
        """
        log_py_Ea = self.log_py_ac0(soft_past, y)
        ...
        for t, (
            ...
            if K > 0:
                ...
                v2dx_t_flat = (v2dx_t if not self.glove else v2dgx_t).stack(("e", "t"), "r")
                ...
                #v_f_log_qv_t = ntorch.cat([C_v_s_log_qv, nC_Zv.repeat("k", self.K - self.Ke)], "k")
                ...
                log_py_ac1_t = self.proj(past).log_softmax("vocab")
                log_py_ac1_t = (
                ...
                if learn:
                    Bt = log_py_Ea_t
                    rewardt = (log_py_z - Bt).detach() * v_s_log_qv_t
