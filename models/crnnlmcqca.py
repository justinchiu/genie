...
from .fns import attn2, concrete_attn, logaddexp
...
class CrnnLmCqca(LvmA):
    def __init__(
        ...
        super(CrnnLmCqca, self).__init__()
    ...
    def pa0(self, emb_x, s, x_info, emb_e, ue_info, emb_t, ut_info, v2dx):
        ...
        if not self.inputfeed:
            ...
            # ea: T x N x R
            log_ea, ea, ec, e_log_alpha = attn2(rnn_o, emb_e, ue_info.mask)
            #log_t, ea_T, ec_T = attn(rnn_o + ec_E, tA, ut_info.mask)
            log_ta, ta, tc, t_log_alpha = attn2(rnn_o, emb_t, ut_info.mask)
            if self.noattn:
            ...
            ea = ea.rename("els", "e")
            ta = ta.rename("els", "t")
            e_log_alpha = e_log_alpha.rename("els", "e")
            t_log_alpha = t_log_alpha.rename("els", "t")
        ...
        return e_log_alpha, ea, ec, t_log_alpha, ta, tc, log_va, va, vc, output, s
    ...
    def log_qac_y(self, emb_y, y_info, emb_e, ue_info, emb_t, ut_info, v2dx):
        ...
        # ea: T x N x R
        log_ea, e_log_alpha, eg = concrete_attn(
            rnn_o, self.Wie(emb_e), ue_info.mask, self.qtao, k=self.K)
        log_ta, t_log_alpha, tg = concrete_attn(
            rnn_o, self.Wit(emb_t), ut_info.mask, self.qtao, k=self.K)
        e_log_alpha = e_log_alpha.rename("els", "e")
        t_log_alpha = t_log_alpha.rename("els", "t")
        log_ea = log_ea.rename("els", "e")
        log_ta = log_ta.rename("els", "t")
        log_va = log_ea + log_ta
        vc = log_va.exp().dot(("e", "t"), v2dx)
        if self.qcrnn:
            #rnn_oc, _ = self.brnnc(emb_y.detach(), None, lengths=y_info.lengths)
            rnn_oc, _ = self.brnnc(emb_y, None, lengths=y_info.lengths)
        log_c = self.Wicopy(rnn_oc if self.qcrnn else rnn_o).log_softmax("copy")

        self.eg = eg
        self.tg = tg

        return log_c, vc, e_log_alpha, t_log_alpha, log_ea, log_ta
    ...
    def forward(
        ...
        if self.noattnvalues:
            past = self.Wc_nov(ntorch.cat([rnn_o, ec, tc], "rnns")).repeat("k", 1).tanh()
        elif self.nuisance:
            past = rnn_o.repeat("k", 1).rename("rnns", "ctxt")
        else:
            past = self.Wc(ntorch.cat([rnn_o, ec, tc, vc], "rnns")).repeat("k", 1).tanh()
        log_py_Ea = self.log_py_ac0(past, y)

        if y is not None:
            emb_y = self.lutx(y)
            # log_qe and log_qt are the log alphas of the concrete dist
            # UNNORMALIZED
            log_qc, vc, log_qe, log_qt, e_s, t_s = self.log_qac_y(
        ...
        for t, (
            ...
            vc_t = vc_t._new(vc_t.detach().values.requires_grad_(True))
            e_s_t = e_s_t._new(e_s_t.detach().values.requires_grad_(True))
            t_s_t = t_s_t._new(t_s_t.detach().values.requires_grad_(True))
            ...
            if K > 0:
                ...
                if self.qc:
                    ...
                    # log_p and log_q are the unnormalized log alphas
                    lpe_t = self.log_prob(log_pe_t - self.tao * e_s_t, ue_info.lengths, "e")
                    lqe_t = self.log_prob(log_qe_t - self.qtao * e_s_t, ue_info.lengths, "e")
                    lpt_t = self.log_prob(log_pt_t - self.tao * t_s_t, ut_info.lengths, "t")
                    lqt_t = self.log_prob(log_qt_t - self.qtao * t_s_t, ut_info.lengths, "t")

                    e_log_ratio = lpe_t - lqe_t
                    t_log_ratio = lpt_t - lqt_t

                    Eqa_log_py_a_t = logaddexp(
                        log_py_c0_t + math.log(0.5),
                        log_py_ac1_t + e_log_ratio + t_log_ratio + math.log(0.5),
                    )
                    nll_t = -Eqa_log_py_a_t.mean("k")[y_maskt].sum()
                    log_py_z = Eqa_log_py_a_t

                    kl_e_t = self.kl(
                        log_qe_t.softmax("e"),
                        log_qe_t.log_softmax("e"), log_pe_t.log_softmax("e"),
                        ue_info.lengths, "e",
                    ).sum()
                    kl_t_t = self.kl(
                        log_qt_t.softmax("t"),
                        log_qt_t.log_softmax("t"), log_pt_t.log_softmax("t"),
                        ut_info.lengths, "t",
                    ).sum()
                    kl_t = 0
                    klc_t = 0
    ...
    # for concrete dist
    def log_prob(self, delta, lens, dim):
        lp = []
        for i, N in enumerate(lens.tolist()):
            deltai = delta.get("batch", i).narrow(dim, 0, N)
            lp.append(
                (N-1) * math.log(self.tao) + deltai.sum(dim) - N * deltai.logsumexp(dim)
           )
        return ntorch.stack(lp, "batch")
